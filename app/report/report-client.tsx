...
export function ReportClient() {
  ...
  const exportMarkdown = () => {
    const lines = [
      `# GEOCITATION report: ${result.domain}`,
      "",
      `Scanned: ${formatDate(result.scannedAt)}`,
      `Score: ${result.score}/100 (${result.grade})`,
      `Pages sampled: ${result.metrics.pagesScanned}`,
      `Signals checked: ${result.metrics.signalsChecked}`,
      "",
      "## Executive brief",
      "",
      `GEOCITATION found ${issues.length} priority issues and ${strengths.length} verified strengths across crawler access, entity confidence, extractability, structured data, trust and machine-readable guidance.`,
      "",
      "## Priority fixes",
      "",
      ...(executiveIssues.length ? executiveIssues.flatMap((finding) => [
        `- ${finding.code}: ${markdownBullet(finding.action)}`,
        `  Evidence: ${markdownBullet(finding.evidence)}`,
      ]) : ["- No priority fixes were detected in this scan."]),
      "",
      "## Quick wins",
      "",
      ...(quickWins.length ? quickWins.map((finding) => `- ${finding.code}: ${markdownBullet(finding.action)}`) : ["- Re-run the scan after implementation and move into model-specific visibility testing."]),
      "",
      ...
      result.methodology,
      "",
      "Generated by GEOCITATION.",
    ];
    downloadFile(`${safeFilename(result.domain)}-signal-executive-brief.md`, lines.join("\n"), "text/markdown;charset=utf-8");
  };
  ...
  return (
    <main className="report-page" id="main-content">
      ...
      <section className="report-cover" id="overview">
        ...
        <div className="report-cover-copy">
          ...
          <div className="report-cover-actions">
            ...
            <button type="button" onClick={exportMarkdown}>Client brief <span>↓</span></button>
      ...
      <section className="report-brief" id="brief">
        <div className="report-section-head">
          <span>00 / EXECUTIVE BRIEF</span>
          <div><h2>What a decision maker <span>needs first.</span></h2><p>This section is built for client delivery: the risk, the fastest wins, and the strengths worth preserving before deeper prompt testing starts.</p></div>
        </div>
        <div className="brief-grid">
          <article>
            <span>PRIORITY FIXES</span>
            <strong>{executiveIssues.length}</strong>
            {executiveIssues.length ? executiveIssues.map((finding) => <p key={finding.code}><b>{finding.code}</b>{finding.action}</p>) : <p><b>READY</b>No priority fixes were detected in this scan.</p>}
          </article>
          <article>
            <span>QUICK WINS</span>
            <strong>{quickWins.length}</strong>
            {quickWins.length ? quickWins.map((finding) => <p key={finding.code}><b>{finding.code}</b>{finding.action}</p>) : <p><b>NEXT</b>Re-run after implementation and move into prompt visibility testing.</p>}
          </article>
          <article>
            <span>STRENGTHS TO PROTECT</span>
            <strong>{protectedStrengths.length}</strong>
            {protectedStrengths.length ? protectedStrengths.map((finding) => <p key={finding.code}><b>{finding.code}</b>{finding.evidence}</p>) : <p><b>BUILD</b>No durable strengths were detected yet.</p>}
          </article>
        </div>
      </section>
      ...
      <footer className="report-footer">
        <Link className="brand footer-brand" href="/"><img className="brand-logo" src="/geocitation-logo.png" alt="GEOCITATION" /></Link>
