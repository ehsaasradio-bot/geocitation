...
export async function POST(request: Request) {
  ...
  if (typeof payload?.reportId !== "string" || !/^[a-f0-9]{32}$/.test(payload.reportId) || typeof payload.promptKey !== "string" || payload.provider !== "openai") return Response.json({ error: "Invalid provider run." }, { status: 400 });
  const availability = await openAIAvailability();
  if (!availability.connected) return Response.json({ error: "OpenAI test credentials are not configured." }, { status: 503 });
  ...
  try {
    const run = await runOpenAIVisibilityTest(report, prompt.text, (await ownerKey(auth.user.email)).slice(0, 64));
